...
See the `extension wiki page <http://www.tipfy.org/wiki/extensions/i18n/>`_.
...
or ``auto_now_add`` are set to `True` in ``db.Model`` properties:
``db.DateProperty``, ``db.DateTimeProperty`` or ``db.TimeProperty``.
