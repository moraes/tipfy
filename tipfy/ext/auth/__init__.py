...
from tipfy import (cached_property, get_config, get_request, Forbidden,
    import_string, local, redirect, RequestRedirect, url_for)
    ...
    def login_with_session(self):
        ...
        if session.get('to_signup', None):
            ...
            if not _is_auth_endpoint('signup_endpoint'):
                raise RequestRedirect(create_signup_url(get_request().url))
...
def login_required(func):
    ...
    def decorated(*args, **kwargs):
        ...
        return redirect(create_login_url(get_request().url))
...
def user_required(func):
    ...
    def decorated(*args, **kwargs):
        ...
        if is_authenticated():
            ...
            return redirect(create_signup_url(get_request().url))
        else:
            # Redirect to login page.
            return redirect(create_login_url(get_request().url))
...
def basic_auth_required(validator):
    ...
    def wrapper(func):
        def decorated(*args, **kwargs):
            return validator(get_request().authorization, func, *args,
                **kwargs)
